...
TidyTensors support `ranknames()` in addition to traditional `dimnames()` (implemented as `names(dimnames())`) for meaningful rank annotation. Let's consider the CIFAR10 dataset distributed with `keras`. Actually, let's consider a sample of these data distributed directly with TidyTensor:
...
This appears to be a set of 5000 32x32 RGB images, in a channels-last organization. We can set ranknames with either `ranknames(t) <-` syntax or the tidy-friendly `set_ranknames()`, and the `.dots` parameter can be used to provide ranknames as a character vector.
